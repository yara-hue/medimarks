      ...
      <Container>
        ...
        <div className="max-w-3xl mx-auto space-y-3">
          {faqs.map((faq, index) => (
            <div
              key={index}
              className="bg-gray-50 dark:bg-navy-800/50 rounded-xl border border-gray-200/60 dark:border-navy-700 overflow-hidden"
            >
              <button
                onClick={() => setOpenIndex(openIndex === index ? null : index)}
                className="w-full flex items-center justify-between p-5 md:p-6 text-left transition-colors hover:bg-gray-100 dark:hover:bg-navy-800"
              >
