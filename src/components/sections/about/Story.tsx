  ...
  return (
    <section className="py-24 md:py-32 bg-white">
      <Container>
        ...
        >
          <span className="text-navy-500 font-medium text-sm tracking-widest uppercase">Our Story</span>
            ...
            {timeline.map((item, index) => (
              ...
              >
                <div className={`hidden md:block w-1/2 ${index % 2 === 0 ? "text-right pr-12" : "pl-12"}`}>
                  <span className="font-heading font-bold text-4xl text-navy-200">{item.year}</span>
                </div>

                <div className="relative flex items-center justify-center shrink-0">
                  <div className="w-8 h-8 rounded-full bg-navy-500 border-4 border-white shadow-md" />
                </div>

                <div className={`md:w-1/2 ${index % 2 === 0 ? "md:pl-12" : "md:pr-12 md:text-right"}`}>
                  <span className="md:hidden font-heading font-bold text-lg text-navy-500">{item.year}</span>
                  <h3 className="mt-1 font-heading font-semibold text-xl text-navy-900">{item.title}</h3>
